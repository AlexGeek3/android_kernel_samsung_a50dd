...
void decon_abd_save_log_event(struct abd_protect *abd, const char *print)
{
	...
	abd->event.log[idx].stamp = ktime_to_ns(ktime_get());
...
static void decon_abd_enable_interrupt(struct decon_device *decon, struct abd_pin *pin, bool on)
{
	...
	decon_info("%s: on: %d, %s(%d) level: %d, count: %d, state: %d%s\n", __func__,
		on, pin->name, pin->irq, pin->level, trace->count, decon->state, (pin->level == pin->active_level) ? ", active" : "");
		...
		irqreturn_t func(int irq, void *dev_id))
{
	...
	if (!gpio_is_valid(gpio)) {
		decon_info("%s: gpio_is_valid fail, gpio: %d\n", __func__, gpio);
		goto exit;
	}
...
irqreturn_t decon_abd_con_handler(int irq, void *dev_id)
{
	...
	disable_irq_nosync(decon->abd.con_irq);
