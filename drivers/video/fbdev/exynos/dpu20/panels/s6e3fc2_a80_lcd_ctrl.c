...
static int s6e3fc2_read_coordinate(struct lcd_info *lcd)
{
	int ret = 0;
...
static int s6e3fc2_read_manufacture_info(struct lcd_info *lcd)
{
	int ret = 0;
	unsigned char buf[LDI_GPARA_MANUFACTURE_INFO_CELL_ID + LDI_LEN_MANUFACTURE_INFO_CELL_ID] = {0, };

	ret = s6e3fc2_read_info(lcd, LDI_REG_MANUFACTURE_INFO_CELL_ID, ARRAY_SIZE(buf), buf);
	if (ret < 0)
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
...
#if 0
static int s6e3fc2_read_elvss(struct lcd_info *lcd)
{
	int ret = 0;
	unsigned char buf[LDI_LEN_ELVSS] = {0, };

	ret = s6e3fc2_read_info(lcd, LDI_REG_ELVSS, LDI_LEN_ELVSS, buf);
	if (ret < 0)
		dev_err(&lcd->ld->dev, "%s: fail\n", __func__);
...
static int s6e3fc2_read_init_info(struct lcd_info *lcd)
{
	...
	s6e3fc2_read_coordinate(lcd);
	s6e3fc2_read_manufacture_info(lcd);
	s6e3fc2_read_chip_id(lcd);
//	s6e3fc2_read_elvss(lcd);
