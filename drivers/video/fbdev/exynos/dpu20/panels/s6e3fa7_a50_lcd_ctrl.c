...
#if defined(CONFIG_EXYNOS_DECON_LCD_EA8076_A50)
static int s6e3fa7_module_init(void)
{
	struct device_node *node;
	struct property *prop;
	__le32 *list;

	node = of_find_node_with_property(NULL, "lcd_info");
	prop = of_find_property(node, "lcd_info", NULL);
	...
	if (lcdtype & BIT(6)) {
		...
		prop->value = list;
		prop->length /= 2;
		register_lcd_driver(&s6e3fa7_mipi_lcd_driver);
	}

	return 0;
}
module_init(s6e3fa7_module_init);
#endif
