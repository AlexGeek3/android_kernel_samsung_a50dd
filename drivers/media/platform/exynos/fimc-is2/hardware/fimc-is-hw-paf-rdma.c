...
extern atomic_t        g_pafstat_rsccount;
...
static int fimc_is_hw_paf_enable(struct fimc_is_hw_ip *hw_ip, u32 instance, ulong hw_map)
{
	...
	if (!atomic_read(&g_pafstat_rsccount)) {
		msinfo_hw("%s: hw_com_init()\n", instance, hw_ip, __func__);
		pafstat_hw_com_init(hw_paf->paf_core_regs);
	}
	atomic_inc(&g_pafstat_rsccount);

	msinfo_hw("%s : (rsccount):(%d)\n", instance, hw_ip,
		__func__, (u32)atomic_read(&g_pafstat_rsccount));
...
static int fimc_is_hw_paf_disable(struct fimc_is_hw_ip *hw_ip, u32 instance, ulong hw_map)
{
	...
	if (atomic_read(&g_pafstat_rsccount) > 0)
